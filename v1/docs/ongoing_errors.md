...
| **008** | `T_dedup = 0.96` — proposition semantic deduplication merge threshold (single source of truth in `worker/extract/dedup.py`, Item W1, Item D2 & Item D8); ambiguous-band adjudication does not earn its cost; re-pointing guarded by entailment validation (`T_ENTAIL_HIGH = 0.70`); indexical propositions prohibited (Item W0); canonical noun-phrase matter at issue enforced (Item D1); position-bearing matter at issue enforced (Item D6); position elicited with proposition via prompt `v1.8` and stored `position_frame` (Item X2); calibrated against stored frames ground truth (Item D8) | Phase 2 / P0 / W1 / W0 / C1 / D1 / D2 / D6 / X2 / D8 | **Toward merging, guarded by frame identity, entailment, and self-containedness.** Re-measured September 10, 2026 over live corpus ($n = 1,508$ unmerged propositions, 1,517 claims, prompt `v1.8` with canonical noun-phrase matter at issue and stored `position_frame`, 23 sources). 1-NN cosine similarity distribution ($n=1,507$ active propositions): min 0.6184, max 0.9843, mean 0.7619, std 0.0599, median 0.7522; deciles D10 0.6948, D20 0.7132, D30 0.7261, D40 0.7382, D50 0.7522, D60 0.7694, D70 0.7860, D80 0.8087, D90 0.8425, D100 0.9843. Evaluated against stored position frames as ground truth: pairwise similarities of four named defect pairs from §12: Sacks growth 0.8518, FDA drug approval 0.8494, China open source 0.8772, Friedberg interest rate 0.9515. Threshold $T_{\text{dedup}} = 0.96$ selected above D95 (superseding 0.84): eliminates 100% of frame-contradicted merges on named defect pairs and Support/Oppose propositions (1/1 clean); preserves genuine punctuation and surface restatements ($\ge 0.96$). Active propositions: 1,507 (1,499 singletons, 99.47%; 8 multi-claim propositions). |
...
| **031** | `delta = 0.05` — stance direction margin (Validator 7, Items S1 / §17n, D3 / §17t, D4 / §13w, X2); augmented with scope-aware syntactic negation analysis (governance of proposition predicate, continuation modals, epistemic verbs, discourse markers, conversational qualifiers); standing bidirectional correction counters (`stance_corrected_to_support`, `stance_corrected_to_oppose`); `hedge` stance literal retired in favor of `hedging_level: float` on `Literal["support", "oppose", "mixed"]` | S1 (§17n) / D3 (§17t) / D4 (§13w) / X2 | **Directional entailment and scope-aware syntactic governance.** Sentence embeddings represent negation weakly ($sim(Q, P) \approx sim(Q, \neg P)$ within $\pm 0.005$), while un-scoped syntactic negation matches negators indiscriminately across quotes (causing a near 100% false-flip rate on live support claims containing negation, Item D4). Under Item D4, `has_syntactic_negation` enforces syntactic scope governance over proposition predicates while excluding non-scoping idioms, double-negative continuation modals (*"not going to stop"*), epistemic verbs (*"never predict that"*), attenuating qualifiers (*"not that much of an increase"*), and contrastive foil subclauses. Re-measured under Item X2 over drawn random sample from live corpus ($n=80$ own-assertion claims, seed 168): confusion matrix shows True Support: 75/75 (100.0%) ended Support, 0/75 (0.00%) ended Oppose (0.00% false-flip rate); True Oppose: 5/5 (100.0%) ended Oppose, 0/5 (0.00%) ended Support; zero confusion errors across all 80 cases. All 4 canonical quotes end as `support`. Live corpus ($n=1,517$ claims): support 1,228 (80.9%), oppose 278 (18.3%), mixed 11 (0.7%). |
...
| **033** | `MIN_CLAIMS_PER_HOUR = 2.0` — source claims-per-hour rate floor (Item D5 / §13x, re-derived Item X4 / §11); replaces C1's zero-floor rule (`no source contributes zero claims`) with an audio-duration rate check in `worker/integrity.py` (`verify_claims_per_hour`) | D5 (§13x) / X2 / X4 | **Conservative rate floor guarding against silent truncation and starved sources.** Derived from empirical 23-source distribution under Prompt v1.9: panel/debate roundtables yield 4.65 to 24.65 claims/hr; solo-host guest Q&A interviews where the host asserts positions (e.g. `79f3aaf4ae50dde5` Intel CEO interview) yield 2.41 claims/hr; verified pure guest Q&A interviews where the host acts purely as moderator/interviewer (`04ff0000906a6d10` Mark Cuban interview, 41.6 min, 103 candidate utterances all evaluated and cleanly declined under Rule 0) yield 0.00 claims/hr and are permitted 0 claims when verified. Re-derived under Item X4 across all 23 sources: observed range 0.00 – 24.65 claims/hr across 401 claims (100% pass rate). |
...
|---|---|---|
| **035** | **B** — **measure the ceiling before deciding.** 100 already-extracted utterances through one frontier model with the v1.9 prompt unchanged, both outputs scored by the same hand-read test. An experiment, not a migration: nothing in `worker/` changes and no corpus is re-extracted. Answers whether five format failures were the format or the model. | `design_claim_extraction.md` §6 · `agent_execution_guide.md` M1 |
