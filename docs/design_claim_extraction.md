...
Negation in a proposition does a second kind of damage. Validator 7 decides stance direction partly by looking for negation in the quote; when the **proposition** also carries a negator, a quote that agrees with it reads as opposing it — measured on live data, that is one of the two causes of a `support` → `oppose` false-flip rate near 100% on the claims sampled. **Enforce it in the extraction prompt *and* in a validator: reject any proposition text matching polarity patterns at canonicalisation time with reason `proposition_carries_polarity`. A rule the model can violate silently is not a rule.

**Enforced in Item D1 (§13u):** The polarity validator was extended to positive and modal polarity (bare `should`, `must`, `ought`), evaluative comparatives (`better/worse/cheaper/faster/stronger than`), outcome predictions (`is/are favored to`, `will win/beat`), and subordinate clause negation (`not`, `n't`, `neither`, `nor`). Rejection reason is standardized to `proposition_carries_polarity`. Extraction prompt was updated to `v1.6` (`gemma-3-27b-it:v1.6:s1`) restoring canonical noun-phrase matters at issue.

**Enforced in Item D6 (§11):** The position-bearing floor is guarded mechanically via `validate_position_bearing` in `worker/extract/validators.py` (reason: `proposition_not_position_bearing`). It rejects propositions under 5 words, bare noun phrases lacking relational markers (preposition, participle, comparative), finite root verbs, bare verb openers, question openers, and unbound trailing pronouns. The extraction prompt was updated to `v1.7` (`gemma-3-27b-it:v1.7:s1`) incorporating Rule 2's position test with negative examples drawn from the live database. Re-extraction over the live 23-source corpus pruned bare-topic propositions (reconciled claim shortfall: 583 bare-topic rejections), eliminated all 6 false candidate pairs from D2, and preserved historical quarantined fabrications with zero published fabrications.

**Enforced in Item X2 (§12):** Elicited the position with the proposition via prompt `v1.8` (`gemma-3-27b-it:v1.8:s1`) under Issue 034 Option B. The model writes the full position frame (`the speaker is FOR <X>` / `the speaker is AGAINST <X>`), stored directly in `claims.position_frame`. ⟨X⟩ becomes `proposition_text` and the frame dictates `stance`. Stored frame makes position test directly auditable from the table rather than generous re-judgement. Re-extracted full corpus yielding 1,517 claims across 1,464 active propositions; 20/20 random claims pass byte-identical agreement with proposition and stance. Dual falsification verified: varying ⟨X⟩ with stance loses reversal candidates under self-join, while byte-identical ⟨X⟩ preserves them; prompt reversion to v1.7 on 20 utterances increases Validator 2b fire rate from 11.1% back to 22.2% baseline.
...
**Enforced in Item W0:** `validate_self_contained` runs immediately after quote resolution and before embedding/entailment in `validate_extracted_claim`. Rejection reason is `proposition_not_self_contained`. The extraction prompt was updated to `v1.3` (`gemma-3-27b-it:v1.3:s1`) with Rule 3 explicitly banning indexical and reflexive frames and providing negative/positive few-shot pairs. Tension detection also enforces Precondition 6 (quarantining tensions on indexical propositions).

**Completed against the property in Item W2 (§17p):** W0 specified a narrow list of observed failures rather than the general principle. In W2, the validator was rewritten against the fundamental principle: *a proposition must be globally interpretable and evaluable standing entirely on its own, without requiring external conversational context*. Rejects sentence-initial pronouns/deictics (`It is...`, `This...`, `That...`, `They...`), unbound third-person pronouns (`they/their`, `he/his/him`), and comparatives with no relatum (`the same`, `such`, `the other`), while preserving bound pronouns with internal antecedents (`Moderna patented its mRNA technology`). The extraction prompt was updated to `v1.5` (`gemma-3-27b-it:v1.5:s1`). All ~130 affected claims across 132 propositions were purged, re-extracted under `v1.5` and validated, leaving exactly zero unbound propositions in the active store.
...
7. **Stance Direction Check — the directional half of entailment.** *(Added September 5, 2026 after four candidate contradiction pairs were traced to mislabelled stances or rhetorical setups — Item S1, §17n; augmented September 6, 2026 with syntactic negation analysis and bidirectional correction — Item D3, §17t.)*
   ...
   **Mechanism:** Sentence embeddings represent negation weakly ($sim(Q, P) \approx sim(Q, \neg P)$ within $\pm 0.005$), which caused legacy Validator 7 to only ever correct in one direction (9 oppose $\to$ support, 0 support $\to$ oppose; Trap 56). Under Item D3, Validator 7 introduced bidirectional correction via syntactic negation detection; but un-scoped matching falsely flipped support claims containing non-scoping negation (*"not going to stop"*, *"never predict"*). Under Item D4 (§13w), Validator 7 incorporates syntactic scope analysis (`has_syntactic_negation`):
   - For `oppose`: If quote contains syntactic negation whose scope governs the proposition predicate or an oppositional predicate, it is confirmed oppose. If quote contains NO scoping negation and $sim(Q, P) > sim(Q, \neg P)$, quote asserts $P$ rather than opposing it, correcting to `support` (incrementing standing counter `stance_corrected_to_support`) or rejecting with `rejection_reason: stance_direction_mismatch`.
   - For `support`: If quote contains syntactic negation genuinely scoping over the proposition predicate (and not part of continuation modals, epistemic verb qualification, discourse markers, or conditional/relative clauses), quote asserts $\neg P$ rather than supporting $P$, correcting to `oppose` (incrementing standing counter `stance_corrected_to_oppose`) or rejecting with `rejection_reason: stance_direction_mismatch`.
   - Standing bidirectional counters: `stance_corrected_to_support` and `stance_corrected_to_oppose`.
   - Evaluated against a drawn evaluation sample ($n=80$, seed 168) with a 0.00% false-flip rate on support claims, 4/4 canonical quotes ending as `support`, and zero confusion errors.
...
**Resolved:** Issue 007 → local Gemma for both stages (§6). Issue 005 → `nomic-embed-text-v1.5` at 768 dims for dedup. Issue 008 (Parameter 008) → `T_dedup = 0.96` calibrated against stored frames ground truth over prompt v1.8 distribution ($n=1,508$ active propositions, September 10, 2026, Item D8 superseding D2 and P0); separates all 4 named defect pairs, 100% clean support/oppose frame identity; ambiguous-band adjudication does not earn its cost.
