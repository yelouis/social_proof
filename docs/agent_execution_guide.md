...
| `mypy --strict` | **PASS on 95 files** | Clean across worker/, tests/, fixtures/, golden/, scripts/. Item G1, W0 & S1 delivered. |
| `pytest tests/ -q` | **PASS** — 213 passed in **220s** | `requires_models` tests ran (not skipped, no deselection in `addopts`). All 213 unit, behavioural, and falsification tests pass. |
...
| **Corpus — claims** | **1,362 CLAIMS (N0, P0, W0, S1 DELIVERED)** | Extraction ran across all 4,219 utterances. Old indexical claims deleted and re-extracted under `v1.3` prompt (`gemma-3-27b-it:v1.3:s1`) with `validate_self_contained` active. Validator 7 (`validate_stance_direction`) and I7 speech act sensitivity active in prompt `v1.4` and validator chain. 2 mislabelled oppose claims corrected to support, 97 interrogative/rhetorical claims excluded. All 4 sources contribute. |
...
| **Candidate pairs** | **1 REMAIN (4 ELIMINATED BY S1)** | All 4 target candidate pairs identified in §17n eliminated by Validator 7 and I7 speech-act exclusion. Exactly 1 opposing-stance candidate pair remains in corpus: Pair 5 ("We should do the same thing on AI", containing residual unbound "the same"), addressed under Item W2. |
| **`stance`** | **VALIDATED (S1 DELIVERED)** | Validator 7 (`validate_stance_direction`) certifies directional alignment ($P$ vs $\neg P$) with margin $\delta = 0.05$. Inverted oppose claims (e.g. Friedberg spending `af95392de868a188` and Sacks AI tasks `7f571f16d81af8c5`) corrected to support. Genuine oppose claims survive ($n=73 \ge 50$). |
| **`is_own_assertion`** | **SENSITIVITY RAISED — 7.78% (S1 DELIVERED)** | 106 of 1,362 claims excluded (7.78%, up from 0.66%). 96 interrogatives (`exclusion_reason="question"`), 1 rhetorical setup (`exclusion_reason="hypothetical"`), 9 ambiguous entailment. Measured via `get_exclusion_rate()`. |
