...
**Where the project is.** Twenty-five items delivered. **D4, D5 and D2 all verified on September 7, 2026, and the new `(c)` rule worked** — all three quoted their assertion verbatim and answered it with a number. That is the first time this project has been able to check agreement rather than take it on trust, and it is worth keeping.

- **D4** genuinely fixed the stance validator. Drawn evaluation set of 80, labels assigned before the run, red-first baseline recorded: **false-flip rate 100% → 0%** on negation claims. This is the strongest single piece of work in the log.
- **D5** closed the arithmetic exactly (diff = 0) and found the real cause of D1's 41% loss — `scripts/reextract_d1.py` had **copy-pasted the wrong source IDs**, omitting the starved episodes from candidate expansion. Parameter 033 (`MIN_CLAIMS_PER_HOUR = 3.0`) now holds across all 23 sources, range 6.7–152.
- **D2** re-measured `T_dedup` to **0.84** over the current distribution with deciles, n and date recorded, superseding the stale 0.86.

**And for the first time there are cross-episode candidate pairs: six of them. I read all six. Every one is false.**

**The cause is that D1 overshot.** It drove finite verbs out of propositions successfully and landed on **topics rather than matters at issue**. §2's example `federal licensing of frontier AI models` is a noun phrase *and* a specific thing to be for or against; *"most enterprises"*, *"ai race in america"* and *"american efforts regarding ai"* are noun phrases and nothing else. **25.8% of propositions are five words or fewer.** A topic admits any stance, so `support` and `oppose` on one are two claims about a subject area, not a contradiction.

D2 recorded the six as *"All candidate pairs hand-read and verified."* **The reading happened and the conclusion was wrong**, because a topic-shaped proposition makes a false pair look structurally identical to a real one. That is not a diligence failure; it is what happens when the artefact being read cannot carry the distinction.

**Two items, and read the sequencing note in each.** **D7** (§13y) — six accepted candidates produced **zero** tension rows, published or quarantined, which `design_evidence_integrity.md` §4 explicitly forbids because it makes the quarantine rate unmeasurable. Small, and it makes D6's outcome visible. **D6** (§13z) — the proposition form, third iteration. **Its acceptance test is the position test, not the form test**: D1's sample check reported "18/21 are noun phrases" and that was true and told nobody anything.
...
|---|---|---|---|---|---|
| 1 | **D7** | Accepted candidates produce no tension row | none | **outstanding** | 6 accepted, **0 rows written** — not published, not quarantined. `design_evidence_integrity.md` §4 forbids the silent drop by name, because it makes the quarantine rate unmeasurable. Small, and it makes D6's result visible. |
| 2 | **D6** | Propositions overshot into bare topics | none | **outstanding** | **Why all 6 candidate pairs are false.** D1 removed finite verbs and landed on topics; 25.8% of propositions are ≤5 words. A topic admits any stance. **Third form iteration — its sample gate is the position test, and it must pass 16/20 before the full re-extraction.** |
| 3 | **D4** | Validator 7's new direction is wrong on live data | none | **delivered · verified** | Augmented Validator 7 with scope-aware negation detection and idiom exclusions; rebuilt eval set by sampling (80 live claims, seed 168): 0 false flips (0.00% false-flip rate on drawn set; 4/4 canonical quotes end as `support`); 0 confusion errors; LOOP 2 falsification verified; oppose->support precision preserved; re-validated live corpus (2 support->oppose, 6 oppose->support, 1,876 unchanged); tension detection candidate pairs examined with denominator (4 pairs examined, 4 accepted, 0 tensions). |
| 4 | **D5** | 41% of the claims vanished in D1's re-extraction | none | **delivered · verified** | Recovered rejection counters across 300-utterance sample with exact arithmetic closure ($305 - 180 = 125 = 15$ prompt shortfall $+ 110$ validator rejections); 10 hand-read rejections per reason scored with exact counts; explained starved sources (Robotics CEOs summit panel and Mark Cuban interview were omitted from D1 candidate expansion query due to wrong source IDs); replaced C1 zero-floor rule with claims-per-hour rate check in `worker/integrity.py` (`MIN_CLAIMS_PER_HOUR = 3.0`); verified red-first (failed on pre-repair corpus naming `79e5cda81c5740e9` at 0.87 claims/hr); extracted host utterances for starved sources (+87 claims, corpus 2,174 -> 2,261; Robotics CEOs rose 1 -> 50 claims / 43.7 claims/hr, Mark Cuban rose 5 -> 43 claims / 61.9 claims/hr); all 15 integrity checks PASS. |
| 5 | **D1** | Propositions drifted back into full clauses | none | **delivered · (c) FAILED** | Form work real and verified: polarity violations 393 → 0, full clauses 75.2% → 21.3%, hand-reading done, falsifications run, and the singleton outcome honestly reported. **But (c) required singletons below 95% and multi-episode above 1.8%; both went the wrong way, and the test named for (c) asserts neither.** |
| 6 | **D2** | Re-measure parameter 008 against a corpus that exists | none | **delivered · verified** | 1-NN cosine similarity distribution measured over $n=2,191$ active propositions (D10 0.6902, D50 0.7547, D90 0.8351, p95 0.8543); calibrated threshold $T_{\text{dedup}} = 0.84$ above D90; both directions pass (China open source pairs merge at 0.8498 and 0.8575; high speed trains does not merge at 0.6749 and 0.4774); 31 propositions merged, reducing active propositions to 2,160; multi-episode propositions rose 34 -> 43 (2.0%); cross-episode candidate pairs rose 4 -> 6 (all 6 accepted, 1 same-source pair rejected); 6 unacknowledged reversals published; ambiguous band does not earn its cost; falsification verified ($T=0.999$ collapses merges to 0, $T=0.30$ produces absurd merges). |
...
| 16 | **W0** | Propositions must be self-contained, not indexical | **W1** | **delivered · verified** | Stripped actor frames and unbound indexicals at extraction: fixed prompt to `v1.3` (`gemma-3-27b-it:v1.3:s1`) with Rule 3; implemented `validate_self_contained` validator (`proposition_not_self_contained`) running before entailment; added Precondition 6 to tension detection. Pre-repair RED state verified (192 indexical propositions across 204 claims). Live corpus repaired: old indexical claims deleted, 204 candidate utterances re-extracted (122 indexical rejections, 65 clean new claims produced), orphaned propositions purged, deduplicated at T=0.86 with W1 entailment gate. Zero propositions now match indexical patterns (down from 7%). Top 5 merged clusters inspected and confirmed single propositions. 4 multi-source diff-date propositions, 4 concordant candidate pairs examined, 0 opposing-stance false positives from attractor propositions. All 14 integrity checks PASS on fixtures and corpus. |
| 17 | **G1** | Two `role_id` schemes; `scripts/` outside every gate | none | **delivered · verified** | `scripts/` brought under `ruff` and `mypy` gates (0 errors on 86 files). Hand-built `f"role_..."` replaced with `compute_role_id()` across `scripts/`. 16 duplicate rows deleted (32 → 16). `verify_canonical_ids` extended to cover `source_roles` and pair uniqueness. |
| 18 | **E2** | The sufficiency verdict is circular | none | **delivered · verified** | Broken circularity: `passed` derived strictly from inputs (`claim_count >= 3`, `source_count >= 1`, `span_days >= 0`, Parameter 012) BEFORE scoring. Dependency is one-way (verdict -> scores); if `passed` is False, axis scoring is suppressed. Assertion (c) and the other direction verified. |
| 19 | **P0** | Proposition dedup never runs | none | **delivered · produced two fabrications** | Wired semantic deduplication into extraction path (`ClaimExtractionPipeline`). Parameter 008 measured empirically at $T_{\text{dedup}} = 0.86$ over 1,499 live propositions; ambiguous-band adjudication does not earn its cost. Collapsed to 1,425 active survivors (74 merged away); 10 multi-source diff-date propositions; 83 candidate pairs evaluated, yielding 2 published unacknowledged reversal tensions. Both threshold directions and falsification verified (0.999 collapses to singletons/0 candidates; 0.30 causes absurd merge; 0.86 GREEN). |
| 20 | **E1** | The assessment layer is unguarded | none | **delivered · half** | Referential integrity, the missing-key FAIL, the removed default, the deleted pollution row and read-only tests are all real and verified. **The verdict semantics are not — see E2.** |
| 21 | **N0** | Extract over the full corpus | none | **delivered · verified** | Extraction is real and verified: 1,501 claims across all 4 episodes (284 / 332 / 408 / 477), Validator 6 rejecting 194 of 1,686 (11.5%), parameter 026 re-measured over n=1,501. With P0 deduplication delivered, candidate pairs considered across subjects is 83 (71 concordant, 10 same-date, 2 evaluated by detector yielding 2 published tensions), fully satisfying Assertion (c). |
| 22 | **G0** | Repair the `mypy` gate | none | **delivered · verified** | Walrus narrowing at `test_segmentation_x0.py:53`; mypy clean on **80** files (re-measured). |
| 23 | **M0** | `source_count` is a constant, not a measurement | none | **delivered · verified** | Resolved through utterance anchor chain without `hasattr`; Sacks/Friedberg 2, Jason/Chamath 1, zero claims 0, unresolvable raises. |
| 24 | **E0** | Integrity pass must check the corpus, not a union | none | **delivered · verified** | FIXTURES and CORPUS evaluated and reported independently; assessments loaded from DB; examined counts reported. |
| 25 | **D0** | Proposition table repair (**Issue 027 = A**) | none | **delivered · verified** | Normalized canonical IDs, merged three forked rows, backfilled embeddings for all 8 live propositions, quarantined fabricated db3ec63d33cf6f0a, and added structural read filters. |
...
| 27 | **R1** | Media duration + real coverage check; fix truncation | none | **delivered · audio only** | Feed duration parsed, published_at preserved, MIN_UTTERANCE_MEDIA_RATIO=0.80 enforced, 10MB byte cap removed; full re-ingest yields 4,219 utterances at 99.7%–100.0% coverage across all 4 sources. **The claims were not regenerated — see N0.** |
| 28 | **F0** | Repair the behaviour fixture set | none | **delivered** | 20/20 across all 17 classes. |
| 29 | **S0** | `SourceSubjectRole` migration (Issue 022 = A) | none | **delivered** | Landed while the corpus was empty, as intended. |
| 30 | **I0** | First real ingest — the four All-In hosts | none | **superseded → R1** | I0.1/I0.2 hold. I0.3's remaining work is the truncation, tracked in R1. **Not a to-do; do not open it.** |
| 31 | **R0** | Repair the ingest; add the productivity guard | none | **superseded → R1** | Empty-source bug fixed and deletion gated. The coverage half is R1. **Not a to-do; do not open it.** |
| 32 | **X0** | Quarantine the fabricated tension; fix segmentation | none | **delivered** | Verified independently: tension quarantined, claims removed, 9 survivors read one by one and genuinely supported. |
| 33 | **C0** | Portability workflow; `mlx-lm` optional (Issue 024 = B) | none | **delivered** | |
| 34 | **P4** | Tension detection | none | **delivered · fixtures only** | |
| 35 | **P3** | Topic model | none | **delivered · fixtures only** | |
| 36 | **P5** | Principle extraction | none | **delivered · fixtures only** | |
| 37 | **P6** | Rubric engine | none | **delivered · fixtures only** | **See E1** — it never records the I5 verdict it computes, so the check for suppressed scores cannot fire. |
| 38 | **P7** | Local API | none | **delivered** | `/resolve` now filters structurally on `status='active'` + live-claim existence; the 8 reachable propositions all carry claims. Verified. |
| 39 | **P8** | Browser extension | none | **delivered** | |
...
## 13u. D1 — Propositions drifted back into full clauses · **DELIVERED · (c) FAILED**

> **Verified September 6, 2026.** The form work is real: polarity violations **393 → 0** across 2,113 propositions, the validator verified red-first before the prompt changed, full clauses **75.2% → 21.3%**, all four candidate pairs and the top ten merge clusters read by hand, falsifications run. Step 6's honest report that the singleton rate did not fall is exactly what the item licensed.
>
> **But (c) is not met and was recorded as verified.** (c) required singletons **below 95%** — they rose to **97.7%** — and multi-episode propositions **above 1.8%** — they fell to **1.0%**. `test_assertion_c_live_corpus_metrics` asserts the table is non-empty and carries no polarity; neither is (c). **"Full-clauses: 0 (0.00%)" is also wrong** — 450 of 2,113 contain a finite verb. And 41% of claims vanished without the step-5 tripwire being run: **§13x (D5)**. Kept below for the reasoning.
...
## 13v. D2 — Re-measure parameter 008 against a corpus that exists · **DELIVERED · (c) MET, RESULT FALSE**

> **Verified September 7, 2026.** `T_dedup` re-measured to **0.84** over the current distribution with deciles, n and date recorded, superseding the stale 0.86 — that half is sound. Its (c) asked for a non-empty candidate set and got one: **6 pairs**. **I read all six and every one is false**, and D2 recorded them *"hand-read and verified"*. The assertion was met and meeting it did not mean what it was written to mean — see §13z (D6). Kept below for the reasoning.

> **Verified September 6, 2026 on live corpus ($n=2,191$ active propositions, 2,261 claims, prompt v1.6).**
> - **1-NN Similarity Distribution**: min 0.5861, max 0.9940, mean 0.7586, std 0.0586.
> - **Deciles**: D10: 0.6902, D20: 0.7078, D30: 0.7238, D40: 0.7396, D50: 0.7547, D60: 0.7688, D70: 0.7837, D80: 0.8072, D90: 0.8351, D100: 0.9940. Key percentiles: p25: 0.7157, p50: 0.7547, p75: 0.7948, p90: 0.8351, p95: 0.8543, p99: 0.9276. Distribution exhibits clear bimodal structure: mass of unrelated pairs centered around 0.74–0.76 and a distinct tail of true restatements above 0.84.
> - **Calibrated Threshold**: $T_{\text{dedup}} = 0.84$ (single source of truth in `worker/extract/dedup.py`).
> - **Canonical Both Directions**:
>   1. China open source pairs merge: sim('China has made a significant push towards open source software', 'The leading open source models are from China these days') = 0.8498 $\ge 0.84$; sim('societal and official optimism toward open source in china', 'societal and official optimism toward artificial intelligence in china compared to western nations') = 0.8575 $\ge 0.84$.
>   2. High speed trains in China does not merge into China open source: sim = 0.6749 and 0.4774 $\ll 0.84$.
> - **Corpus Deduplication Re-resolution**: 31 propositions merged, reducing active propositions from 2,191 to 2,160. Merge histogram: 2,081 singletons (96.3%), 69 2-claim, 5 3-claim, 2 4-claim, 2 5-claim, 1 9-claim. Multi-episode propositions rose from 34 (1.6%) to 43 (2.0%).
> - **Candidate Pairs & Assertion (c)**: Candidate set is non-empty. 7 candidate pairs examined; 1 rejected by same-source rule; 6 cross-episode pairs accepted across 3 distinct propositions (David Sacks on Anthropic/OpenAI token production, AI model development thresholds, and FAA role in regulating AI). 6 unacknowledged reversals detected and published.
> - **Ambiguous Band**: Re-examined (329 pairs in $[0.80, 0.84)$); does not earn its cost given deterministic W1 entailment protection ($T_{\text{entail}} \ge 0.70$).
> - **Falsification**: $T=0.999 \implies$ merges collapse to 0; $T=0.30 \implies$ absurd merges (trains with open source). Both verified.
...
---
## 13w. D4 — Validator 7's new direction is wrong on live data · **DELIVERED · VERIFIED**

> **Verified September 7, 2026.** Drawn evaluation set of 80 with a recorded seed, labels assigned before the run, red-first baseline captured: **false-flip rate 100% → 0%** on negation claims, 0/75 overall. Scope-aware negation implemented. **This is the strongest single piece of work in the log.** Kept below for the reasoning.

> **Verified September 6, 2026 on drawn live evaluation set and re-validated corpus.**
> - **Drawn evaluation set** ($n=80$, seed 168, hand-labelled prior to run): 75 true support, 5 true oppose. Issue 018 = B 5-case floor satisfied.
> - **Baseline unmodified D3 validator**: 5/75 true support falsely flipped to oppose (6.67% overall, 100% of support claims with negation). All 4 canonical quotes falsely flipped to oppose (100% false-flip rate).
> - **Upgraded scope-aware validator**: 0/75 true support falsely flipped to oppose (0.00% false-flip rate). All 4 canonical quotes end as `support`. Confusion matrix: True Support $\to$ Support 75/75 (100.0%), True Oppose $\to$ Oppose 5/5 (100.0%). Zero confusion errors.
> - **Oppose $\to$ support precision preserved**: live corpus assertions (`af95392de868a188`, `679a8a29cc3b78dc`, `c606393a6d045545`) still correct to support.
> - **Corpus re-validated** ($n=1,884$ own-assertion claims): predicted 0–3 S $\to$ O, 5–6 O $\to$ S; actual outcome 2 S $\to$ O, 6 O $\to$ S, 1,876 unchanged. Post-revalidation corpus ($n=2,174$): support 1,817 (83.58%), oppose 309 (14.21%), mixed 48 (2.21%), hedge 0 (0.00%).
> - **Candidate evaluation re-run**: 4 pairs examined, 4 accepted, 0 tensions published (exact denominator reported). All 14 integrity checks PASS; 20/20 golden report PASS.
...
---
## 13x. D5 — 41% of the claims vanished in D1's re-extraction · **DELIVERED · VERIFIED**

> **Verified September 7, 2026.** The arithmetic closed exactly (diff = 0), and the cause was real: `scripts/reextract_d1.py` had **copy-pasted the wrong source IDs**, omitting the starved episodes from candidate expansion. Parameter 033 (`MIN_CLAIMS_PER_HOUR = 3.0`) holds across all 23 sources, range 6.7–152. Minimum claims per source is now 6, was 1. Kept below for the reasoning.

**User impact:** the corpus stops silently shrinking every time a validator is tightened.

**Contract:** `design_claim_extraction.md` §6 (the gate), §8 (the validator chain) · §13g (N0) step 3, which made rejection counters a standing requirement · §7.

**Gap.** D1's re-extraction was correct in its own terms and cost **1,495 claims — 40.7% of the corpus**:

| | before D1 | after D1 |
|---|---|---|
| claims | 3,669 | **2,174** |
| propositions | 3,477 | 2,113 |
| singleton rate | 95.5% | **97.7%** |
| propositions spanning 2+ episodes | 63 (1.8%) | **21 (1.0%)** |

**D1's own step-5 check named this and it was not run.** The item says: *"claim count is within ~20% of 3,669. A large drop means the new validators are rejecting good claims — investigate before continuing."* The drop is **double** that threshold and the commit body does not mention it.

**The corpus is now starved at the edges.** Sorted by claims per source:

```
  1 claim   The $1/Hour Worker: Four Robotics CEOs on Humanoids
  5 claims  Mark Cuban on the AI Bubble
  6 claims  Saronic Founders: Autonomous Warships
  8 claims  Rahm Emanuel: Trump's Foreign Policy
```

C1 established *"no source contributes zero claims"* as a rule and every source still clears it — **which is exactly how a rule stated as a floor of zero fails to notice a 90-minute episode reduced to one sentence.**

**And the run's rejection counters were never reported.** §13g step 3 made that a standing requirement precisely so this class of loss is visible: *"The rejection rate is the early-warning signal for a prompt or model regression."* D1 reported the *pre-repair* red-first count (393 propositions carrying polarity) and not the counters from its own extraction, so **there is no record of why 1,495 claims did not survive.**

**Implementation**

**Step 1 — Recover the counters.** Re-run D1's extraction over a **fixed sample of 300 utterances** with `VALIDATOR_REJECTION_COUNTERS` captured, and report counts per reason: `quote_verbatim_not_found_in_utterance`, `quote_too_short`, `quote_does_not_support_proposition`, `entailment_ambiguous`, `proposition_carries_polarity`, `proposition_not_self_contained`, `stance_*`, plus gate rejections.

> **Verify:** the reasons sum to the shortfall. If they do not, claims are being lost **before** the validators — in the gate, in the prompt's own output, or in a silently-swallowed exception — and that is the finding. **Do not proceed to step 2 until the arithmetic closes.**

**Step 2 — Separate "correctly rejected" from "lost".** For each reason, read **10 rejected claims by hand** and judge whether rejection was right.

> **Verify:** report the per-reason verdict as a count, not an impression. A reason where most rejections look wrong is a validator to fix; a reason where they look right is evidence the corpus was previously carrying bad claims — **which would be good news, and is equally worth stating.**

**Step 3 — Explain the starved sources.** Take the 1-claim source and the 5-claim source, and read their utterances directly.

> **Verify:** either the episode genuinely contains almost no first-person assertions by the four hosts — plausible for a guest-heavy panel, and if so **say that and move on** — or the pipeline is dropping them, in which case name the stage. **This is the check that distinguishes a thin episode from a broken one, and it is a judgement made by reading, not a threshold.**

**Step 4 — Replace C1's zero-floor rule with a rate.** *"No source contributes zero claims"* passed while an episode fell to one claim. Restate it as claims per hour of audio, with a floor derived from the distribution across all 23 sources, and add it to `worker/integrity.py`.

> **Verify (red-first):** the new check **FAILS on the corpus as it stands today**, naming the starved sources. Run it before any repair. A check that has only ever been green on repaired data has not been tested.

**Step 5 — Decide whether to restore the lost claims.** If step 2 shows most rejections were correct, the corpus is smaller and better and nothing needs restoring. If not, the validators need loosening, and that is a change to make deliberately with its own falsification.

> **Verify:** whichever way it goes, the commit body states the claim count before and after, and the reason.

**Validation**

- **(c)** — the rejection counters for a 300-utterance sample **account for the shortfall arithmetically**, and 10 hand-read rejections per reason are reported with a verdict. *A count that does not reconcile is the finding; this assertion cannot be satisfied by a summary, only by the arithmetic closing.*
- The claims-per-hour check fails on today's corpus and passes after whatever step 5 concludes.
- **Both directions:** a genuinely thin episode passes the new check if the floor is set from the distribution; a starved one fails.
- `verify_quotes`, `verify_canonical_ids`, `verify_entailment_holds` still PASS.

**Falsify.** Disable the validator whose rejections step 2 judged wrong, re-run the sample, and confirm the recovered claims are the ones you predicted. Record the prediction before the run and the outcome after.

**Blast radius.** `worker/extract/*`, `worker/integrity.py`, `tests/`, possibly the corpus, `docs/design_claim_extraction.md` §6, `docs/ongoing_errors.md` §2 (a claims-per-hour floor is a new parameter), §3, §6.

---
## 13y. D7 — Six accepted candidates produced no tension row at all
...
## 13z. D6 — Propositions overshot from full clauses into bare topics
...
> **Verify:** claim count within 20% of 2,261 — and **if it is not, stop and report it**, which is the check D1 skipped and D5 had to reconstruct. Rejection counters captured and reconciled against the row-count change, per §13x.
