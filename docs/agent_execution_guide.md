...
| **Candidate pairs** | **0 CANDIDATES (1 EXAMINED, 1 DISQUALIFIED UNDER T1)** | Evaluated with honest denominator via `evaluate_candidate_pairs`: 1 pair examined on live corpus, 1 rejected by same-source rule (`same_source_stance_conflict`), 0 accepted candidates. Synthetic cross-episode pair accepted. LOOP 2 falsified. |
