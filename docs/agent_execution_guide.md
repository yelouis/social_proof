...
**Where the project is.** Twenty items delivered. **A0 and C1 both verified on September 6, 2026** by exercising them, not by reading their commits. A0: a writable `Storage` holding the lock now raises `RuntimeError` from `create_app` instead of silently handing the site a writable cursor — I constructed that exact configuration. C1: **23 sources, 20,666 utterances, 3,669 claims**, every source ≥ 80% coverage, 92 role rows for 23×4, no source with zero claims, the selection rule pre-registered in both the commit and the design doc, and every candidate pair hand-read (7 false reversals found and corrected). 14 integrity checks PASS. **This was well-executed work.**

**And the corpus still cannot contain a finding. A six-fold expansion changed the merge rate by 0.006.**

| | before C1 (1,288 claims) | after C1 (3,669 claims) |
|---|---|---|
| propositions per claim | 0.954 | **0.948** |
| singleton propositions | 95.4% | **95.5%** |
| propositions spanning 2+ episodes | 4 | 63 (**1.8%** of the table) |
| cross-source `support`↔`oppose` pairs | 0 | **0** |

**Issue 030 was the right decision against the wrong diagnosis, and that is worth saying plainly.** Overlap was never limited by corpus size. **75.2% of propositions (2,615 of 3,477) contain a finite verb** — they are full clauses, not the noun-phrase *matter at issue* that `design_claim_extraction.md` §2 calls "the single most important rule". Full clauses naming one matter embed far apart, so `t_dedup = 0.86` merges 4.5% of the table and almost nothing recurs across episodes. Many also carry **positive** polarity (*"Forces should be allowed to play out"*, *"democrats are favored to win"*) — §2 forbids polarity outright, and the validator only checks the negative forms.

**What that means for you.** **D3** (§17t) — validator 7 has corrected stance **9 times, every one `oppose` → `support`, none the other way**; embeddings represent negation weakly, so it may simply be a support-detector, and it feeds the field the detector keys on. **D1** (§17u) — restore the canonical proposition form; this is the root cause. **D2** (§17v) — re-measure parameter 008, which was fitted to a distribution that no longer exists. Read §3 and §5 traps 28–57 first.
...
| **Stance direction** | **9 CORRECTIONS, ALL ONE WAY** | Validator 7 has corrected `oppose`→`support` nine times (2 in S1, 7 in C1) and `support`→`oppose` zero times, while the `oppose` share fell 15.2% → 11.9%. Embeddings represent negation weakly, so `sim(Q,P) > sim(Q,¬P)` holds for almost any quote merely *about* P. **A validator that has never fired the other way has not been shown to discriminate.** Item D3. |
...
| 12 | **W0** | Propositions must be self-contained, not indexical | **W1** | **delivered · verified** | Stripped actor frames and unbound indexicals at extraction: fixed prompt to `v1.3` (`gemma-3-27b-it:v1.3:s1`) with Rule 3; implemented `validate_self_contained` validator (`proposition_not_self_contained`) running before entailment; added Precondition 6 to tension detection. Pre-repair RED state verified (192 indexical propositions across 204 claims). Live corpus repaired: old indexical claims deleted, 204 candidate utterances re-extracted (122 indexical rejections, 65 clean new claims produced), orphaned propositions purged, deduplicated at T=0.86 with W1 entailment gate. Zero propositions now match indexical patterns (down from 7%). Top 5 merged clusters inspected and confirmed single propositions. 4 multi-source diff-date propositions, 4 concordant candidate pairs examined, 0 opposing-stance false positives from attractor propositions. All 14 integrity checks PASS on fixtures and corpus. |
| 13 | **G1** | Two `role_id` schemes; `scripts/` outside every gate | none | **delivered · verified** | `scripts/` brought under `ruff` and `mypy` gates (0 errors on 86 files). Hand-built `f"role_..."` replaced with `compute_role_id()` across `scripts/`. 16 duplicate rows deleted (32 → 16). `verify_canonical_ids` extended to cover `source_roles` and pair uniqueness. |
| 14 | **E2** | The sufficiency verdict is circular | none | **delivered · verified** | Broken circularity: `passed` derived strictly from inputs (`claim_count >= 3`, `source_count >= 1`, `span_days >= 0`, Parameter 012) BEFORE scoring. Dependency is one-way (verdict -> scores); if `passed` is False, axis scoring is suppressed. Assertion (c) and the other direction verified. |
| 15 | **P0** | Proposition dedup never runs | none | **delivered · produced two fabrications** | Wired semantic deduplication into extraction path (`ClaimExtractionPipeline`). Parameter 008 measured empirically at $T_{\text{dedup}} = 0.86$ over 1,499 live propositions; ambiguous-band adjudication does not earn its cost. Collapsed to 1,425 active survivors (74 merged away); 10 multi-source diff-date propositions; 83 candidate pairs evaluated, yielding 2 published unacknowledged reversal tensions. Both threshold directions and falsification verified (0.999 collapses to singletons/0 candidates; 0.30 causes absurd merge; 0.86 GREEN). |
| 16 | **E1** | The assessment layer is unguarded | none | **delivered · half** | Referential integrity, the missing-key FAIL, the removed default, the deleted pollution row and read-only tests are all real and verified. **The verdict semantics are not — see E2.** |
| 17 | **N0** | Extract over the full corpus | none | **delivered · verified** | Extraction is real and verified: 1,501 claims across all 4 episodes (284 / 332 / 408 / 477), Validator 6 rejecting 194 of 1,686 (11.5%), parameter 026 re-measured over n=1,501. With P0 deduplication delivered, candidate pairs considered across subjects is 83 (71 concordant, 10 same-date, 2 evaluated by detector yielding 2 published tensions), fully satisfying Assertion (c). |
| 18 | **G0** | Repair the `mypy` gate | none | **delivered · verified** | Walrus narrowing at `test_segmentation_x0.py:53`; mypy clean on **80** files (re-measured). |
| 19 | **M0** | `source_count` is a constant, not a measurement | none | **delivered · verified** | Resolved through utterance anchor chain without `hasattr`; Sacks/Friedberg 2, Jason/Chamath 1, zero claims 0, unresolvable raises. |
| 20 | **E0** | Integrity pass must check the corpus, not a union | none | **delivered · verified** | FIXTURES and CORPUS evaluated and reported independently; assessments loaded from DB; examined counts reported. |
| 21 | **D0** | Proposition table repair (**Issue 027 = A**) | none | **delivered · verified** | Normalized canonical IDs, merged three forked rows, backfilled embeddings for all 8 live propositions, quarantined fabricated db3ec63d33cf6f0a, and added structural read filters. |
...
| 23 | **R1** | Media duration + real coverage check; fix truncation | none | **delivered · audio only** | Feed duration parsed, published_at preserved, MIN_UTTERANCE_MEDIA_RATIO=0.80 enforced, 10MB byte cap removed; full re-ingest yields 4,219 utterances at 99.7%–100.0% coverage across all 4 sources. **The claims were not regenerated — see N0.** |
| 24 | **F0** | Repair the behaviour fixture set | none | **delivered** | 20/20 across all 17 classes. |
| 25 | **S0** | `SourceSubjectRole` migration (Issue 022 = A) | none | **delivered** | Landed while the corpus was empty, as intended. |
| 26 | **I0** | First real ingest — the four All-In hosts | none | **superseded → R1** | I0.1/I0.2 hold. I0.3's remaining work is the truncation, tracked in R1. **Not a to-do; do not open it.** |
| 27 | **R0** | Repair the ingest; add the productivity guard | none | **superseded → R1** | Empty-source bug fixed and deletion gated. The coverage half is R1. **Not a to-do; do not open it.** |
| 28 | **X0** | Quarantine the fabricated tension; fix segmentation | none | **delivered** | Verified independently: tension quarantined, claims removed, 9 survivors read one by one and genuinely supported. |
| 29 | **C0** | Portability workflow; `mlx-lm` optional (Issue 024 = B) | none | **delivered** | |
| 30 | **P4** | Tension detection | none | **delivered · fixtures only** | |
| 31 | **P3** | Topic model | none | **delivered · fixtures only** | |
| 32 | **P5** | Principle extraction | none | **delivered · fixtures only** | |
| 33 | **P6** | Rubric engine | none | **delivered · fixtures only** | **See E1** — it never records the I5 verdict it computes, so the check for suppressed scores cannot fire. |
| 34 | **P7** | Local API | none | **delivered** | `/resolve` now filters structurally on `status='active'` + live-claim existence; the 8 reachable propositions all carry claims. Verified. |
| 35 | **P8** | Browser extension | none | **delivered** | |
...
---
## 17t. D3 — Validator 7 has only ever corrected in one direction
...
**Validation**
- **(c)** — **validator 7 corrects at least one claim from `support` to `oppose`** over a hand-labelled set of genuine oppositions, or the item reports that it cannot and changes the instrument. *A validator with a 9:0 correction ratio is indistinguishable from one that always answers `support`, and only a case it must get the other way can tell them apart.*
- Both directions on the hand-labelled set: genuine supports stay `support`, genuine opposes end as `oppose`. Report the confusion counts, not an accuracy figure.
- The corpus `oppose` share after re-validation is reported next to its previous value. **Do not target a number** — report what the labelled set says the validator's error rate is.

**Falsify.** Feed the validator a quote that plainly negates its proposition and confirm it labels `oppose`. If it does not, that is the finding and the item's conclusion is "the instrument is wrong", which is a legitimate delivery. Record it.

**Blast radius.** `worker/extract/validators.py`, `fixtures/behaviour/`, `docs/design_claim_extraction.md` §2, `docs/ongoing_errors.md` §2 (031 re-measured; `hedge` resolved), §3, §6.
