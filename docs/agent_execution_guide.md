...
| **Stance direction** | **DELIVERED · VERIFIED (D3 DELIVERED)** | Validator 7 augmented with syntactic negation analysis; standing bidirectional correction counters (`stance_corrected_to_support`, `stance_corrected_to_oppose`); 12 hand-labelled cases verified with 4 support $\to$ oppose corrections (Assertion c) and 2 oppose $\to$ support corrections with 0 confusion errors; LOOP 2 falsification verified; `hedge` stance literal retired in favor of `hedging_level: float` on `Literal["support", "oppose", "mixed"]`; single legacy hedge claim migrated to support (0 hedge rows remain in DB); 240/240 tests pass. |
