...
|---|---|---|---|---|---|
| 1 | **D4** | Validator 7's new direction is wrong on live data | none | **outstanding** | **Unblocked, and now the unblocker.** D1 produced the first 4 cross-source `support`↔`oppose` candidates this corpus has ever had, and read all four: every one is a false flip from negation without scope. Fix this and the detector has real candidates. |
