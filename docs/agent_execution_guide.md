...
**Where the project is.** Twenty-three items delivered. `HEAD` delivers Item D5 (§13x): 2,261 claims, 2,192 propositions (2,191 active, 1 quarantined), 20,666 utterances, 23 sources. Gates green, tree clean, 250 tests passing, 15 integrity checks PASS.

**The queue is D2.** D5 (§13x) recovered the rejection counters from D1's 41% drop with exact arithmetic closure, separated good pruning from lost assertions with 10 hand-read cases per reason, diagnosed the starved sources (source ID omission in D1 expansion query), added `verify_claims_per_hour` (Parameter 033 `MIN_CLAIMS_PER_HOUR = 3.0`) verified red-first, and extracted host utterances for the starved sources (+87 claims, Robotics CEOs rising 1 -> 50 claims, Mark Cuban 5 -> 43 claims). Next is **D2** (§13y): re-measuring `T_dedup` against the current corpus.
...
| **Merge rate** | **MEASURED UNDER D1 NOUN PHRASES** | At existing $T_{\text{dedup}} = 0.86$, singletons remain because $T_{\text{dedup}} = 0.86$ with strict entailment validation ($T_{\text{entail}} \ge 0.70$, Item W1) requires quotes to strictly entail candidate propositions. As §13u Step 6 specifies, this honestly isolates threshold calibration for Item D2. 34 propositions span 2+ episodes. |
...
| **Corpus overlap** | **34 PROPOSITIONS SPAN 2+ EPISODES (D5 DELIVERED)** | Multi-episode noun-phrase propositions span up to 5 episodes. Top clusters verified as single matters at issue without topic blurring. |
...
| **Corpus** | **POPULATED, FULL COVERAGE (R1, N0, P0, W1, W0, C1, D1, D4 & D5 DELIVERED)** | 23 contiguous sources (20 contiguous + 3 historical bootstrap episodes), **20,666 utterances**, **2,261 claims**, **2,192 propositions** (2,191 active, 1 quarantined), 92 roles, 8 assessments. Coverage across all sources >= 80.0% (Parameter 029). All 23 sources clear claims-per-hour rate floor >= 3.0 claims/hr (Parameter 033). |
| **Propositions** | **2,191 ACTIVE (D1 & D5 DELIVERED · VERIFIED)** | Extracted under prompt `v1.6` with canonical noun-phrase matter at issue and extended polarity validation. Consolidated at $T_{\text{dedup}} = 0.86$ with strict re-point entailment validation (`T_ENTAIL_HIGH = 0.70`). Zero unbound pronouns or indexicals. 0% finite verbs. Falsification verified. |
...
| 4 | **D2** | Re-measure parameter 008 against a corpus that exists | none | **outstanding** | `T_dedup = 0.86` merges 4.5% of the table and was fitted to a 1,499-proposition corpus that was 7% indexical attractors, replaced twice since. |
