...
**Where the project is.** Eighteen items delivered. U1 delivered and verified: the review site is served live from a read-only DuckDB connection on the existing local API, four routes, no build step, no generated files. S1, T1 and W2 all verified correct on September 5, 2026 by querying the live system: stance is validated by a seventh validator, the own-assertion guard fires at **8.2%** (was 0.7%), propositions carry no unbound pronouns, and same-source pairs are disqualified from reversal. **1,288 claims, every quote verbatim-verified.**
