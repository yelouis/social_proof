...
| **008** | `T_dedup = 0.86` — proposition semantic deduplication merge threshold (single source of truth in `worker/extract/dedup.py`, Item W1); ambiguous-band adjudication does not earn its cost; re-pointing guarded by entailment validation (`T_ENTAIL_HIGH = 0.70`) | Phase 2 / P0 / W1 | **Toward merging, guarded by entailment.** Over-splitting hides every contradiction, silently. Measured empirically over live corpus ($n = 1,434$ total propositions, 1,501 claims): $T = 0.86$ merges genuine restatements (China open source sim = 0.8632) while keeping distinct topical claims separate. Under Item W1, re-pointing is strictly gated on quote entailment: any claim whose quote does not clear `T_ENTAIL_HIGH = 0.70` against the target proposition refuses merge, keeping its own proposition. Corpus re-resolution merged away 69 propositions (69 claims re-pointed; 8 candidate claim merges refused due to sub-0.70 entailment), leaving 1,430 active propositions carrying claims (plus 3 zero-claim, 1 quarantined fabrication = 1,434 total). 6 multi-source diff-date propositions. Ambiguous-band adjudication does not earn its cost. Provisional until 5-case floor. |
