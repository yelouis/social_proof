...
| **008** | `T_dedup = 0.86` — proposition semantic deduplication merge threshold (single source of truth in `worker/extract/dedup.py`, Item W1); ambiguous-band adjudication does not earn its cost; re-pointing guarded by entailment validation (`T_ENTAIL_HIGH = 0.70`); indexical propositions prohibited (Item W0) | Phase 2 / P0 / W1 / W0 / C1 | **Toward merging, guarded by entailment and self-containedness.** Over-splitting hides every contradiction, silently. Re-measured over 23 contiguous sources ($n = 3,477$ total propositions, 3,476 active, 1 quarantined, 3,669 claims): multi-episode propositions rose from 4 to 67 (56 spanning 2 episodes, 11 spanning 3 episodes), satisfying Assertion (c). Re-pointing strictly gated on quote entailment (`T_ENTAIL_HIGH = 0.70`). Zero propositions match indexical patterns. Ambiguous-band adjudication does not earn its cost. Provisional until 5-case floor. |
