...
| **008** | `T_dedup = 0.86` — proposition semantic deduplication merge threshold (single source of truth in `worker/extract/dedup.py`, Item W1); ambiguous-band adjudication does not earn its cost; re-pointing guarded by entailment validation (`T_ENTAIL_HIGH = 0.70`); indexical propositions prohibited (Item W0); canonical noun-phrase matter at issue enforced (Item D1) | Phase 2 / P0 / W1 / W0 / C1 / D1 | **Toward merging, guarded by entailment and self-containedness.** Over-splitting hides every contradiction, silently. Re-measured over D1 re-extracted corpus ($n = 2,112$ active propositions, 2,174 claims, prompt `v1.6` with canonical noun-phrase matter at issue and extended polarity validation, 0% finite verbs, 0 polarity violations): singletons remain 2,063 of 2,112 (97.68%) because $T_{\text{dedup}} = 0.86$ requires quotes to strictly entail candidate propositions ($\ge 0.70$), leaving threshold retuning to Item D2. 28 propositions span 2+ episodes (top clusters span 3–5 episodes). 4 cross-episode candidate pairs evaluated by TensionDetector. Ambiguous-band adjudication does not earn its cost. Provisional until 5-case floor. |
...
| **031** | `delta = 0.05` — stance direction margin (Validator 7, Items S1 / §17n, D3 / §17t); augmented with syntactic negation analysis; standing bidirectional correction counters (`stance_corrected_to_support`, `stance_corrected_to_oppose`); `hedge` stance literal retired in favor of `hedging_level: float` on `Literal["support", "oppose", "mixed"]` | S1 (§17n) / D3 (§17t) | **Directional entailment and speech-act sensitivity.** Sentence embeddings represent negation weakly ($sim(Q, P) \approx sim(Q, \neg P)$ within $\pm 0.005$), which caused legacy Validator 7 to only ever correct in one direction (9 oppose $\to$ support, 0 support $\to$ oppose, Trap 56). Under Item D3, augmented instrument combines syntactic negation analysis with document embeddings: quote with negation opposing proposition corrects `support` $\to$ `oppose` (incrementing `stance_corrected_to_oppose`); quote without negation asserting proposition corrects `oppose` $\to$ `support` (incrementing `stance_corrected_to_support`). Re-measured over 12 hand-labelled cases ($\ge 5$ per class): 4 support $\to$ oppose corrections, 2 oppose $\to$ support corrections, 0 confusion errors; Assertion (c) verified. Live corpus claims ($n=3,669$): support 3,020 (82.31%), oppose 437 (11.91%), mixed 212 (5.78%), hedge 0 (0.00%). Single legacy hedge claim migrated to support with hedging_level=0.7. Provisional until 5-case floor. |
