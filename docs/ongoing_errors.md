...
| **033** | `MIN_CLAIMS_PER_HOUR = 3.0` — source claims-per-hour rate floor (Item D5 / §13x); replaces C1's zero-floor rule (`no source contributes zero claims`) with an audio-duration rate check in `worker/integrity.py` (`verify_claims_per_hour`) | D5 (§13x) / X2 | **Conservative rate floor guarding against silent truncation and starved sources.** Derived from empirical 23-source distribution: guest-dominated interview panels yield 6.7 to 7.5 claims/hr, whereas starved sources fall below 1.0 claim/hr. Verified under Item X2 across all 23 sources: observed range 15.01 – 89.98 claims/hr across 1,517 claims (100% pass rate). Provisional until 5-case floor. |
