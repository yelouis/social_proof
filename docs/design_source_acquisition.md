...
- **Coverage gate — real media coverage.** The span of extracted utterances must cover at least `min_ratio` (provisional 80%, Parameter 029) of the source media's true duration (`span_ms / duration_ms >= min_ratio`). Duration is sourced from external feed metadata (`<itunes:duration>`), never from the downloaded file artifact, preventing self-confirming truncation traps (Trap 34).
